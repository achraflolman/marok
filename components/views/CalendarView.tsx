...
const CalendarView: React.FC<CalendarViewProps> = ({ userEvents, t, getThemeClasses, tSubject, language, showAppModal, userId, user }) => {
  ...
  const handleSaveEvent = async () => {
    ...
    try {
        if (editingEvent) {
            const eventRef = db.doc(`artifacts/${appId}/users/${userId}/calendarEvents/${editingEvent.id}`);
            await eventRef.update({ ...eventData, updatedAt: Timestamp.now() });
            showAppModal({ text: t('success_event_updated') });
        } else {
            const collectionRef = db.collection(`artifacts/${appId}/users/${userId}/calendarEvents`);
            await collectionRef.add({ ...eventData, createdAt: Timestamp.now() });
  ...
  const handleDeleteEvent = (event: CalendarEvent) => {
    ...
    showAppModal({
        ...
        confirmAction: async () => {
            try {
                const eventRef = db.doc(`artifacts/${appId}/users/${userId}/calendarEvents/${event.id}`);
