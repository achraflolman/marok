...
const NotesView: React.FC<NotesViewProps> = ({ userId, user, t, tSubject, getThemeClasses, showAppModal }) => {
  ...
  useEffect(() => {
    ...
    if (filterSubject === 'all') {
      q = notesCollectionRef.orderBy('createdAt', 'desc');
    } else {
      q = notesCollectionRef.where('subject', '==', filterSubject).orderBy('createdAt', 'desc');
    }
  ...
  const handleSaveNote = async () => {
    ...
    try {
      if (editingNote) {
        const noteRef = db.doc(`artifacts/${appId}/users/${userId}/notes/${editingNote.id}`);
        await noteRef.update({ title, content, updatedAt: Timestamp.now() });
        showAppModal({text: t('note_updated_success')});
        setEditingNote(null);
      } else {
        const collectionRef = db.collection(`artifacts/${appId}/users/${userId}/notes`);
  ...
  const handleDeleteNote = (id: string) => {
    if (user.uid === 'guest-user') {
     ...
     showAppModal({
      ...
      confirmAction: async () => {
        const noteRef = db.doc(`artifacts/${appId}/users/${userId}/notes/${id}`);
