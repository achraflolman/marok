...
const AdminView: React.FC<AdminViewProps> = ({ user, t, tSubject, getThemeClasses, handleLogout, showAppModal, onUserClick, adminSettings, onAdminSettingsUpdate, onPinDisableRequest }) => {
    ...
    const fetchAllData = useCallback(async (tab: string) => {
        ...
        try {
            ...
            if (tab === 'broadcasts') {
                const broadcastsCollectionRef = db.collection(`artifacts/${appId}/public/data/broadcasts`);
                const q = broadcastsCollectionRef.orderBy('createdAt', 'desc');
                const querySnapshot = await q.get();
                setBroadcasts(querySnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as BroadcastData)));
            }
            if (tab === 'feedback') {
                const feedbacksCollectionRef = db.collection(`artifacts/${appId}/public/data/feedback`);
                const q = feedbacksCollectionRef.orderBy('createdAt', 'desc');
                const querySnapshot = await q.get();
            ...
            confirmAction: async () => {
                ...
                try {
                    ...
                    for (const u of allUsers) {
                        ...
                        const notifSnapshot = await notifsRef.where('type', '==', 'admin').get();
                    ...
                    const broadcastCollectionRef = db.collection(`artifacts/${appId}/public/data/broadcasts`);
                    const broadcastsSnapshot = await broadcastCollectionRef.get();
